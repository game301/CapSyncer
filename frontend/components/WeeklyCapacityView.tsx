...
export function WeeklyCapacityView({ coworkerId, coworkerName, year: initialYear }: WeeklyCapacityProps) {
  ...
  const fetchWeeklyData = useCallback(async () => {
    setLoading(true);
    try {
      const response = await fetch(`${apiBaseUrl}/api/capacity/weekly/${coworkerId}/${year}`);
      if (response.ok) {
    ...
    <div className="space-y-6">
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      {isCurrentWeek && (
                        <span className="text-xs font-bold text-blue-300">NOW</span>
                      )}
                    </div>
                    ...
                    <div className="mt-2 text-xs text-white">
                      <div className="font-semibold">{week.usedHours}h / {week.capacity}h</div>
                    ...
                    {week.assignmentCount > 0 && (
                      <div className="mt-1 text-xs text-slate-200">
                        {week.assignmentCount} {week.assignmentCount === 1 ? "task" : "tasks"}
                      </div>
                    )}
                  </div>
                );
      ...
      >
        {selectedWeek !== null && (() => {
          ...
          if (!week) return <div className="text-slate-400">No data for this week.</div>;
          ...
          const weekAssignments = assignments.filter(
            (a) => a.coworkerId === coworkerId && a.year === year && a.weekNumber === selectedWeek
          );
              ...
              <div className="rounded-lg bg-slate-800 p-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <div className={getUtilizationTextColor(week.utilizationPercentage)}>
                      {week.usedHours}h ({week.utilizationPercentage.toFixed(1)}%)
                    </div>
                  </div>
                  <div>
                    <div className="text-sm text-slate-400">Available</div>
                    <div className={week.availableHours < 0 ? "text-red-400" : "text-green-400"}>
                ...
                <div className="mt-4 h-4 w-full overflow-hidden rounded-full bg-slate-700">
                  <div
                    ...
                    style={{ width: `${Math.min(week.utilizationPercentage, 100)}%` }}
                  ></div>
                </div>
              </div>
                        ...
                        >
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                                ...
                                {task && (
                                  <>
                                    ...
                                    <div>
                                      ...
                                      <span className="text-white">{task.weeklyEffort}h</span>
                                    </div>
                                  </>
                                )}
                                <div>
                                  <span className="text-slate-400">Assigned by:</span>{" "}
                                  <span className="text-white">{assignment.assignedBy}</span>
                                </div>
                              </div>
                              {assignment.note && (
                              ...
                              {task?.note && (
                                <div className="mt-2 rounded bg-slate-700 p-2 text-sm text-slate-300">
                                  <span className="font-semibold text-slate-400">Task Note:</span>{" "}
                                  {task.note}
                                </div>
                              )}
                            </div>
