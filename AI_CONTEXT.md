...
> **Purpose:** Track recent work sessions for continuity between AI sessions.
> **Maintenance:** Add new entries at the top. Keep only last 2-3 significant changes. Archive old entries after 30 days.

### March 5, 2026 - API Route Standardization & Backend Logging

**Route Renaming (Breaking Change):**

- Renamed all `/api/capacity/*` routes to `/api/calendar/*` for better semantic clarity
- Frontend route changed: `/capacity` page → `/calendar` page
- Updated: Backend routes, frontend components, tests, HTTP test file, documentation
- All 112 tests passing ✅

**Backend Logging Implementation:**

- Replaced 10 `Console.WriteLine` with structured `ILogger<Program>` logging
- Added comprehensive logging to ALL 26 API endpoints (was 0/26, now 26/26)
- Database initialization now fully logged with proper error handling
- All endpoints have try-catch blocks with LogError, LogWarning, LogInformation
- Ready for production monitoring with Aspire Dashboard and Azure Application Insights

**Async/Await Fix:**

- Changed `db.Database.Migrate()` to `await db.Database.MigrateAsync()` in database initialization
- Prevents blocking startup thread during migrations

**Files Modified:**

- backend/Program.cs, frontend (Navbar, page.tsx, app/calendar/), tests, documentation
